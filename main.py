...
bot = Bot(token="6425067247:AAH3pFKr_5DgpuG2kv-htY20o7jc7Y3EK_0")
...
print('БОЛЬШЕ ТГ БОТОВ - CONFF.ORG')
...
async def send_welcome(message):
    ...
    if str(adm_id) == str(message.from_user.id):
        ...
        inline_btn_5 = InlineKeyboardButton('📌Добавить канал', callback_data='add_channel')
        
        inline_btn_6 = InlineKeyboardButton('📌Удалить канал', callback_data='delete_channel')
        
        inline_btn_7 = InlineKeyboardButton('📌Удалить номер фильма', callback_data='delete_number')
        
        inline_kb1.add(inline_btn_1).add(inline_btn_2).add(inline_btn_3).add(inline_btn_4).add(inline_btn_5).add(inline_btn_6).add(inline_btn_7)
        
        await bot.send_message(chat_id = message.from_user.id,text = '*Админ панель*',parse_mode = 'Markdown',reply_markup = inline_kb1)
...
async def vote_up_cb_handler(call):
    ...
    if c == 1:
       ...
       await bot.send_message(call.from_user.id, text = '*Привет это  БОТ | Поиск🔍\nЧтобы узнать название фильмов из ЮТУБ | ТИК ТОК вам нужно ввести код фильма.\nНажмите на «🔎Поиск» 👇*',parse_mode = 'Markdown',reply_markup = start.greet_kb1)
...
async def send_welcome(message):
    ...
    if c == 1:
        ...
        if str(message.text) == '🔎Поиск':
            ...
            with open(film_cd, 'r' ,encoding="UTF-8") as file:
                ...
                for line in lines:
                    try:
                        ...
                        print("Ссылка на фото:", photo_link)
                    except:
                        print("Ссылка на фото:", photo_link)

            await bot.send_message(chat_id=message.from_user.id, text=number +' '+ title + '\n' + photo_link,parse_mode='Markdown')
